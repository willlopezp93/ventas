...
$(document).on('click','#buscar',function(e){
    ...
    $.post(baseurl+"Articulo/precios",{tipo:tipo,cadena:cadena},function(data){
      $('#tbl_maestro_articulos').html(data);
    });
});
$(document).on('click','#excel_precios',function(){
	window.open(baseurl+"Tablaayuda/excel_precios");
})
...
$(document).on('click','.editar',function(e){
	...
	$('#precio').val(fila.find('td').eq(3).html());
...
if ($('#motivo').val()=='') {
	...
	$.ajax({
		url:baseurl+"Articulo/saveprecio",
		type:"post",
