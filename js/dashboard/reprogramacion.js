...
$(document).ready(function(){
  control_de_menu($('#menu_reprogramacion_chart'));
...
$(document).on('click','.consultar_info',function(){
  $('#dashboard').html('<br><br> <center><img src="http://ventas.codrise.net/assets/img/pageloader.gif">  <h3>Cargando...</h3></center><br> ');
    var mes=$('#mes').val();
          $.post(baseurl+"Charts/reprogramacion/"+mes,function(data){
            $('#dashboard').html(data);
...
$(document).on('click','.modalbutton',function(){
  ...
  var condicion=$(this).attr('data-id');
        $.post(baseurl+"Charts/reprogramacion_detalle/"+mes+"/"+condicion,function(data){
